...
const checkForWinner = function(ii,ij)
{
  ...
  for (let i = ii - 3; i <= ii; i++)  //// Vertical comparison
  {
    if ( i < 0 || i > 2) continue;

    if (Board[i][ij] === Board[i + 1][ij] &&
  ...
  for (let i = ii - 3; i <= ii; i++)  //// Main diagonal
  {
    if ( i < 0 || i > 2) continue;
  ...
  for (let i = ii + 3; i >= ii; i--)  //// Main diagonal Inverse
  {
    if ( i >= 6 || i <= 2) continue;
  ...
  for (let j = ij - 3; j <= ij; j++)  //// Horizontal comparison
  {
    if ( j < 0 || j > 3) continue;
